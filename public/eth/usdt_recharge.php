        ...
        $msg = [
            ...
            'params'=>[
                ...
                [
                    ...
                    'topics'=>['0x2aacc44648e47b83cbc738ef3934a22d80cf48e07a3b514ff6aa8a2be1806b91']
                ]
            ],
        ];
        ...
        if(isset($data['ping'])){
            ...
            if(isset($data['result'])){
                ...
                try {
                    ...
                    // dump($saveData);
                    $recharge = App::make('usdtRecharge')->details(['trade_hash'=>$saveData['trade_hash']]);
                    if(!$recharge){
                        // 充值订单
                        App::make('usdtRecharge')->create($saveData);
                        // 充值流水
                        Factory::UserWalletLogService()->add($saveData['address'], 'usdt', 0, 201, $saveData['amount']);
                    }
                    \DB::commit();
                } catch (\Exception $e) {
                    \DB::rollBack();
                    Log::info('USDT充值异常：'.$e->getMessage());
                }
            }
        }
    };
