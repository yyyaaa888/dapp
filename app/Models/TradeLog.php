...
class TradeLog extends Model
{
    ...
    // 可以被批量赋值的属性
    protected $fillable = ['address','wallet_type','log_type','cashier_type','amount','balance','remarks'];
    ...
    public function getLogNameAttribute()
    {
        if(isset($this->attributes['log_type'])){
            return __($this->logType()[$this->attributes['log_type']]);
        }
    }
    ...
    public function getBalanceAttribute()
    {
        if(isset($this->attributes['balance'])){
            return sctonum($this->attributes['balance']);
        }
    }
    ...
    public function getList($where, $with = [], $page = 1, $page_size = 15)
    {
        ...
        if(isset($where['wallet_type']) && !empty($where['wallet_type'])){
            $model = $model->where('wallet_type',$where['wallet_type']);
        }
