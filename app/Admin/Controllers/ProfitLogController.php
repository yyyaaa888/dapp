...
class ProfitLogController extends AdminController
{
    protected $title = '收益明细';

    public function statistics($collection,$grid)
    {
        $query = App::make('userProfitLog');
    ...
    {
        return Grid::make(App::make('userProfitLog'), function (Grid $grid) {
            // 统计
            $grid->header(function ($query) use ($grid){
                return $this->statistics($query,$grid);
            });

            $logType = App::make('userProfitLog')->logType();
            $cashierType = App::make('userProfitLog')->cashierType();
            ...
            if (Admin::user()->can('profit-log-export')) {
                ...
                $grid->export(['log_id'=>'序号','address'=>'用户地址','log_type'=>'交易类型','cashier_type'=>'收支类型','amount'=>'交易金额','balance'=>'最终余额','remarks'=>'备注','created_at'=>'交易时间'])
                    ->filename('收益明细-'.date('Y-m-d H:i',time()))
    ...
    {
        return Form::make(App::make('userProfitLog'), function (Form $form) {
