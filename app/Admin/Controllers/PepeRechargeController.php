...
class PepeRechargeController extends AdminController
{
    protected $title = 'PEPE充值';

    public function statistics($collection,$grid)
    {
        $query = App::make('pepeRecharge');
    ...
    {
        return Grid::make(App::make('pepeRecharge'), function (Grid $grid) {
            // 统计
            $grid->header(function ($query) use ($grid){
                return $this->statistics($query,$grid);
            });
            $status = App::make('pepeRecharge')->status();
            ...
            if (Admin::user()->can('profit-log-export')) {
                ...
                $grid->export(['recharge_id'=>'序号','address'=>'用户地址','type'=>'充值方式','amount'=>'充值数量','status'=>'充值状态','trade_hash'=>'交易哈希','created_at'=>'充值时间'])
                    ->filename('PEPE充值-'.date('Y-m-d H:i',time()))
    ...
    {
        return Form::make(App::make('pepeRecharge'), function (Form $form) {
